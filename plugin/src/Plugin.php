...
class Plugin {
	...
	/** plugin slug */
	const ID = 'simple-registration-for-edd';
	...
	public function add_plugin_links( $links ) {
		...
		$plugin_links = [];
		$plugin_links['srfedd_configure'] = '<a href="' . admin_url( 'edit.php?post_type=download&page=edd-settings&tab=extensions' ) . '">' . __( 'Configure', 'simple-registration-for-edd' ) . '</a>';
	 ...
	 *
	 * @since 1.0.0
	 * @see simple_registration_for_edd()
	 *
	 * @return Plugin
	 */
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You cannot clone instances of %s.', 'simple-registration-for-edd' ), 'Simple registration for EDD' ), '1.0.0' );
	}
	...
	public function __wakeup() {
		...
		/* translators: Placeholders: %s - plugin name */
		_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You cannot unserialize instances of %s.', 'simple-registration-for-edd' ), 'Simple registration for EDD' ), '1.0.0' );
	}
	...
	public function load_translation() {
		...
		// localization
		load_plugin_textdomain( 'simple-registration-for-edd', false, dirname( plugin_basename( __FILE__ ) ) . '/i18n/languages' );
	}
	...
	public function get_file() {
		return dirname( __DIR__ ) . '/simple-registration-for-edd.php';
	}
	...
	private function install() {
		...
		$installed_version = get_option( 'simple_registration_for_edd_version' );
	...
	private function upgrade( $version ) {
		...
		if ( version_compare( $version, '1.0.0', '=' ) ) {
			...
			update_option( 'simple_registration_for_edd_options', 'disabled' );
		}

		// update the installed version option
		update_option( 'simple_registration_for_edd_version', self::VERSION );
	}
